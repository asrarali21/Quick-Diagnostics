    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="max-w-6xl mx-auto w-full px-4 sm:px-6 pt-6">
        <div className="flex items-center justify-between">
          <button onClick={() => navigate(-1)} className="inline-flex items-center gap-2 text-[#7C5CFC] hover:opacity-80" type="button">
            <ArrowLeft className="w-5 h-5" />
            <span className="font-medium">Back</span>
          </button>
          <h1 className="text-[28px] font-semibold text-gray-900">Check out</h1>
          <div className="w-24" />
        </div>
      </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full border border-gray-200 grid place-items-center">
                  <span className="text-xs font-semibold tracking-wide">{order?.lab?.image}</span>
                </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm">
              ...
              <div className="p-4">
                <div className="flex items-center justify-between">
                  <span className="text-gray-900 font-semibold">{order?.patient?.name || 'Patrick Smith'}</span>
                  <div className="flex items-center text-sm text-gray-500">
                    <span>{order?.patient?.bookingforWhom || 'Self'}</span>
                    <span className="mx-3 w-px h-5 bg-gray-200" />
                ...
                <div className="mt-4 pl-4 border-l-4 border-gray-200">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-gray-900 font-medium">{order?.test?.name || 'COVID RT-PCR Test'}</p>
                      <p className="text-sm text-gray-600">1 Test</p>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="font-bold text-gray-900">{currency(order?.test?.price || itemTotal)}</span>
                      <Trash2 className="w-5 h-5 text-gray-400" />
                    </div>
                  </div>
                  <p className="text-sm text-emerald-600 mt-2">E-Reports by 12hrs</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4">
              ...
              <div className="mt-3">
                <div className="text-gray-900 font-semibold">
                  {order?.slot?.date
                    ? new Date(order.slot.date).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric' })
                    : 'Tue, Feb 23, 2022'}
                </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-[0_-8px_24px_-12px_rgba(17,24,39,0.12)]">
        <div className="max-w-6xl mx-auto w-full px-4 sm:px-6 py-4 flex items-center justify-between">
          <button type="button" onClick={() => navigate(-1)} className="h-12 px-2 text-red-500 font-medium hover:text-red-600">
            Cancel
          </button>
          <button onClick={handlePayment} type="button" className="inline-flex items-center justify-center h-12 px-10 rounded-xl bg-[#7C5CFC] text-white font-semibold shadow-[0_12px_24px_-8px_rgba(124,92,252,0.6)]">
            Pay Now
          </button>
        </div>
      </div>
